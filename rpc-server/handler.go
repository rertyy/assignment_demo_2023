...
func getRoomID(chat string) (string, error) {
	...
	if len(senders) != 2 {
		err := fmt.Errorf("invalid Chat ID '%s', not in the format of user1:user2", chat)
		return "", err
	}
...
func validateSendRequest(req *rpc.SendRequest) error {
	...
	if len(senders) != 2 {
		err := fmt.Errorf("invalid Chat ID '%s', not in format of user1:user2", req.Message.GetChat())
		return err
	}
